    ...
    public function jsTokenMarkup(\StdClass $token, array $args = []): string
    {
        ...
        if ($token->type === 'card' && !empty($token->card->brand) && is_string($token->card->brand)
            ...
            if ($token->card->brand === 'Visa') {
                $_icon = '<i class="visa icon _m _r1r"></i>';
            } elseif ($token->card->brand === 'MasterCard') {
                $_icon = '<i class="mastercard icon _m _r1r"></i>';
            } elseif ($token->card->brand === 'American Express') {
                $_icon = '<i class="american express icon _m _r1r"></i>';
            } elseif ($token->card->brand === 'Discover') {
                $_icon = '<i class="discover icon _m _r1r"></i>';
            } elseif ($token->card->brand === 'JCB') {
                $_icon = '<i class="japan credit bureau icon _m _r1r"></i>';
            } elseif ($token->card->brand === 'Diners Club') {
                $_icon = '<i class="diners club icon _m _r1r"></i>';
            } else { // Just indicate that it's a credit card.
                $_icon = '<i class="credit card alternative icon _m _r1r"></i>';
            }
            ...
            $markup = '<i class="yen icon _m _r1r"></i> <tt data-token="'.$this->c::escAttr(json_encode($token)).'">'.$this->c::escHtml($token->alipay_account->username).'</tt>';
            //
        } elseif ($token->type === 'bank_account' && !empty($token->bank_account->last4) && is_string($token->bank_account->last4)) {
            $markup = '<i class="university icon _m _r1r"></i> <tt data-token="'.$this->c::escAttr(json_encode($token)).'">…••••'.$this->c::escHtml($token->bank_account->last4).'</tt>';
            //
        } elseif ($token->type === 'source_bitcoin' && !empty($token->bitcoin->address) && is_string($token->bitcoin->address)) {
            $markup = '<i class="bitcoin icon _m _r1r"></i> <tt data-token="'.$this->c::escAttr(json_encode($token)).'">'.$this->c::escHtml($token->bitcoin->address).'</tt>';
            //
        } else { // Generic default display using Stripe icon.
            $markup = '<i class="stripe icon _m _r1r"></i> <tt data-token="'.$this->c::escAttr(json_encode($token)).'">'.$this->c::escHtml($token->id).'</tt>';
        }
        return $markup;
    }
