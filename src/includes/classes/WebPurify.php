...
class WebPurify extends AbsBase
{
    ...
    public function __construct(
        ...
        $this->cache_dir = $this->FsDir->normalize($cache_dir);

        if (!$this->cache_dir || !is_dir($this->cache_dir)) {
            $this->cache_dir = $this->FsDir->tmp().'/webpurify';
        }
    }
    ...
    public function check(string $text): bool
    {
        ...
        $endpoint = $this->UrlQuery->addArgs($endpoint_args, $endpoint);

        # Determine sharded cache directory and file.

        $cache_dir  = $this->cache_dir.'/'.$this->Sha1Mod->shardId($endpoint);
        ...
        $response = $this->UrlRemote->request($endpoint, $request_args);
