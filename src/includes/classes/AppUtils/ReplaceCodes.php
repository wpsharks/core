...
class ReplaceCodes extends Classes\AbsBase
{
    ...
    public function __invoke(
        ...
        foreach ($this->Utils->ArrayDotKeys->get($___raw_vars) as $_key => $_value) {
            ...
            if (!is_scalar($_value)) {
                if ($implode_non_scalars) {
                    $_value = (array) $_value;
                    $_value = $this->Utils->ArrayDimensions->one($_value);
        ...
        if ($urlencode && mb_stripos($string, '%%__query_string__%%') !== false) {
            $string = preg_replace_callback('/%%__query_string__%%/ui', function () use (&$___vars) {
                return $this->Utils->UrlQuery->build($___vars);
            }, $string);
        }
        $_iteration_counter = 0; // Check completion every 5th iteration to save time.

        foreach ($___vars as $_key => $_value) {
            $string = preg_replace_callback('/%%'.$this->Utils->RegexQuote($_key).'%%/ui', function () use (&$maybe_urlencode, &$_value) {
        ...
        if (mb_strpos($string, '%%/') !== false) { // Watered-down regex-based codes?
            ...
            $string = preg_replace_callback(
                ...
                function ($m) use (&$maybe_urlencode, &$___vars, &$___var_keys) {
                    ...
                    if (!($keys = $this->Utils->RegexPattern->in($regex, $___var_keys, true))) {
                    ...
                    if ($m['delimiter'] === '&' && $m['key_delimiter'] === '=') {
                        return $this->Utils->UrlQuery->build($values);
                    }
                    if ($m['key_delimiter']) {
