...
class Email extends Classes\AbsBase implements Interfaces\EmailConstants
{
    ...
    public function __invoke($to, string $subject, string $message, array $headers = [], array $attachments = [], bool $throw = false)
    {
        ...
        if ($this->Utils->Html->is($message)) {
            $message_html = $message; // HTML already.
            $message_text = $this->Utils->HtmlConvert->toText($message);
        } else {
            $message_text = $message; // Text already.
            $message_html = $this->Utils->TextConvert->toHtml($message);
        }
        if (!$message_text) { // Set a default plain text alternative in this case.
            $message_text = $this->__('To view this email message, open it in a program that understands HTML.');
        }
        ...
        foreach ($possible_addresses as $_address) {
            ...
            if (mb_strpos($_address, '<') !== false && preg_match('/(?:"(?<name>[^"]+?)"\s*)?\<(?<email>.+?)\>/u', $_address, $_m)) {
                if ($_m['email'] && mb_strpos($_m['email'], '@', 1) !== false && (!$strict || $this->isValid($_m['email']))) {
                    ...
                    $_fname             = $_name ? $this->Utils->Name->firstFrom($_name) : '';
                    $_lname             = $_name ? $this->Utils->Name->lastFrom($_name) : '';
     ...
     */
    public function parseHeaders($value, array $refs = [], bool $strict = false): array
    {
        $headers = []; // Initialize parsed headers.

        foreach ($this->Utils->Headers->parse($value) as $_header => $_value) {
                ...
                case 'from':
                    if (array_key_exists('from_name', $refs) && array_key_exists('from_email', $refs)) {
                        if (($_from_addresses = $this->parseAddresses($_value, $strict))) {
                            ...
                            $refs['from_name']  = $this->Utils->Trim($_from->fname.' '.$_from->lname);
                ...
                case 'reply-to':
                    if (array_key_exists('reply_to_name', $refs) && array_key_exists('reply_to_email', $refs)) {
                        if (($_reply_to_addresses = $this->parseAddresses($_value, $strict))) {
                            ...
                            $refs['reply_to_name']  = $this->Utils->Trim($_reply_to->fname.' '.$_reply_to->lname);
     ...
     */
    public function parseAttachments($value): array
    {
